...
set-option global startup_info_version 20270000
        ...
        evaluate-commands %sh{
            ...
            if [ -n "${branch}" ]; then
                ...
                then
                    ...
                    isstaged=$(cd "$(dirname "${kak_buffile}")" && git diff --staged --name-only "$kak_buffile" 2>/dev/null)
                    if [ -n "${isstaged}" ] && [ -n "${ismodified}" ]; then
